...
사용법:
    python hwp_to_hwpx.py "C:\\변환할파일들이있는폴더"
    python hwp_to_hwpx.py "C:\\변환할파일들이있는폴더" --recursive   (하위 폴더까지 포함)
    python hwp_to_hwpx.py "폴더1" "폴더2" "폴더3"                  (여러 폴더 한 번에)

주의:
    - 실행 중 한글 프로그램이 보안 확인 팝업을 띄우며 멈추는 경우가 있습니다.
      그럴 때는 "한글 자동화 보안모듈"로 검색해서 나오는 레지스트리 등록 방법을
      적용하거나, 처음 몇 개 파일은 팝업이 뜰 때 수동으로 눌러 넘겨주세요.
    - 원본 .hwp 파일은 그대로 두고, 같은 이름의 .hwpx 파일을 새로 만듭니다.
    - 이미 .hwpx로 변환된 파일은 건너뜁니다(다시 실행해도 안전).
"""
...
def collect_todo(path, recursive=False):
    """path는 폴더일 수도, .hwp 파일 하나일 수도 있다 (드래그 앤 드롭 대응)."""
    if os.path.isfile(path):
        files = [path] if path.lower().endswith(".hwp") else []
    else:
        pattern = "**/*.hwp" if recursive else "*.hwp"
        files = glob.glob(os.path.join(path, pattern), recursive=recursive)
        files = [f for f in files if not f.lower().endswith(".hwpx")]

    todo = []
    for f in files:
        out_path = os.path.splitext(f)[0] + ".hwpx"
...
def convert_files(todo):
    ...
    for path in todo:
        out_path = os.path.splitext(path)[0] + ".hwpx"
        try:
            hwp.Open(path)
...
def run(paths, recursive=False):
    ...
    paths: 폴더 경로 또는 .hwp 파일 경로의 리스트 (섞여 있어도 됨)."""
    all_todo = []
    for path in paths:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HWP 파일을 HWPX로 일괄 변환")
    parser.add_argument("folders", nargs="+", help="변환할 .hwp 파일이 들어있는 폴더 경로 (여러 개 가능)")
